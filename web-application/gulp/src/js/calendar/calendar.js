Vue.component('timepicker', {
    props: ['options'],
    template: '<input type="text" :value="options.scheduleDay.time.start" >',
    data: {
        value: $(this.$el).val(),
        sibling: {},
        gotTime: {},
    ...
    methods: {
        checkTime: function () {
            ...
            if (self.options.index === 0) {
                if (this.scheduleSettings[1]) {
                    if (this.$root.scheduleSettings[1][this.options.day] && this.$root.scheduleSettings[1][this.options.day].time.start.indexOf(':') !== -1) {
                        this.sibling.init = this.$root.scheduleSettings[1][this.options.day].time;
                        this.sibling.start = this.parseTime(this.$root.scheduleSettings[1][this.options.day].time.start);
                        this.sibling.end = this.parseTime(this.$root.scheduleSettings[1][this.options.day].time.end);
                    }
                }
            }

            if (index === 1) {
                if (this.$root.scheduleSettings[0][this.options.self.options.day] && this.$root.scheduleSettings[0][self.options.day].time.end.indexOf(':') !== -1) {
                    this.sibling.init = this.$root.scheduleSettings[0][self.options.day].time;
                    this.sibling.start = this.parseTime(this.$root.scheduleSettings[0][self.options.day].time.start);
                    this.sibling.end = this.parseTime(this.$root.scheduleSettings[0][self.options.day].time.end);
                }
            }

            if (this.options.startState) {
                this.gotTime = this.parseTime(value);
                this.relative = this.parseTime(this.options.scheduleDay.time.end);
                if (this.relative) {
                    if (this.$root.compareScheduleTimes(this.relative, this.gotTime)) {
                        this.options.scheduleDay.time.start = value;
                    } else {
                        this.options.scheduleDay.time.start = this.options.scheduleDay.time.end;
                        $(this.$el).val(this.options.scheduleDay.time.end);
                    }
                }
                if (this.sibling.start) {
                    if (this.$root.compareScheduleTimes(this.gotTime, this.sibling.start) && this.$root.compareScheduleTimes(this.sibling.end, this.gotTime)) {
                        this.options.scheduleDay.time.start = this.sibling.end.init;
                        $(this.$el).val(this.sibling.end.init);
                    } else {
                        this.options.scheduleDay.time.start = value;
                    }
                } else {
                ...
                this.gotTime = this.parseTime(value);
                this.relative = this.parseTime(this.options.scheduleDay.time.start);
                if (this.relative) {
                    if (this.$root.compareScheduleTimes(this.relative, this.gotTime)) {
                        this.options.scheduleDay.time.end = this.options.scheduleDay.time.start;
                        $(this.$el).val(this.options.scheduleDay.time.start);
                    } else {
                        this.options.scheduleDay.time.end = value;
                    }
                }
                if (this.sibling.end) {
                    if (this.$root.compareScheduleTimes(this.gotTime, this.sibling.start) && this.$root.compareScheduleTimes(this.sibling.end, this.gotTime)) {
                        this.options.scheduleDay.time.end = this.sibling.end.init;
                        $(this.$el).val(this.sibling.end.init);
                    } else {
                        this.options.scheduleDay.time.end = value;
                    }
    ...
    mounted: function () {
        ...
        $(this.$el)
            .pickatime({
                ...
                min: [0, 0],
                max: [23, 59],
                interval: parseInt(self.options.scheduleDay.interval),
                clear: ''
            })
            .val(this.value)
            .trigger('change')
